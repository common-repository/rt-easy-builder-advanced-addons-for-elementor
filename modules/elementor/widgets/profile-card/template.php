					...
					<?php foreach( $settings[ 'team_social' ] as $key => $item ): ?>
						...
						<?php
							...
							if( !empty( $button[ 'url' ] ) ){
								...
								$attributes_escaped = $base->get_render_attribute_string( $name );
								?>
								<li>
									<a <?php echo $attributes_escaped; // PHPCS:Ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
										<?php if( $button[ 'nofollow' ] ): ?>
											rel="nofollow"
										<?php endif; ?>
									>
										<i class="<?php echo esc_attr( $item[ 'team_social_icon' ] ); ?>"></i>
									</a>
								</li>
								<?php
							}
