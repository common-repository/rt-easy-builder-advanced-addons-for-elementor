...
<div class="wrap rise-builder-importer-wrapper">
	...
	<div class="ocdi rise-builder-importer">
		<h2 class="ocdi__title"><?php esc_html_e( 'Rise Demo Importer', 'rt-easy-builder' ); ?></h2>
		<p class="notice-text"><?php esc_html_e( 'Importing a demo site and customizing it is a great way to save your development time. The demo sites can be imported from the demo listing "demo import" button below', 'rt-easy-builder' ); ?></p>
		<?php
			// Display warrning if PHP safe mode is enabled, since we wont be able to change the max_execution_time.
			if ( ini_get( 'safe_mode' ) ) {
				printf(
					/* translators: %1$s: opening html tag for style  */
					/* translators: %2$s: opening strong tag  */
					/* translators: %3$s: closing strong tag */
					/* translators: %4$s: closing html tag for style  */
					esc_html__( '%1$sWarning: your server is using %2$sPHP safe mode%3$s. This means that you might experience server timeout errors.%4$s', 'rt-easy-builder' ),
			...
			<?php if ( ! empty( $categories ) ) : ?>
				<div class="ocdi__gl-header  js-ocdi-gl-header">
						...
						<ul>
							<li class="active">
								<a href="#all" class="ocdi__gl-navigation-link  js-ocdi-nav-link">
									<?php esc_html_e( 'All', 'rt-easy-builder' ); ?>
					...
					<div clas="ocdi__gl-search">
						<input type="search" class="ocdi__gl-search-input  js-ocdi-gl-search" name="ocdi-gl-search" value="" placeholder="<?php esc_html_e( 'Search demos...', 'rt-easy-builder' ); ?>">
					</div>
				</div>
			<?php endif; ?>
				...
				<?php foreach ( $predefined_themes as $index => $import_file ) : ?>
					...
					>
						<?php if( $import_file[ 'pro' ] ): ?>
							<div class="ribbon"><span><?php esc_html_e( 'premium', 'rt-easy-builder' ); ?></span></div>
						<?php endif; ?>
						<div class="ocdi__gl-item-image-container">
							...
							<?php else : ?>
								<div class="ocdi__gl-item-image  ocdi__gl-item-image--no-image">
									<?php esc_html_e( 'No preview image.', 'rt-easy-builder' ); ?>
								</div>
							<?php endif; ?>
						</div>
						<div class="ocdi__gl-item-footer<?php echo ! empty( $import_file['preview_url'] ) ? '  ocdi__gl-item-footer--with-preview' : ''; ?>">
							...
							<div class="rt-builder-btn-wrapper">
								...
								<?php if ( ! empty( $import_file['preview_url'] ) ) : ?>
									<a class="ocdi__gl-item-button  button preview-btn" href="<?php echo esc_url( $import_file['preview_url'] ); ?>" target="_blank"><?php esc_html_e( 'Preview', 'rt-easy-builder' ); ?><span class="dashicons dashicons-welcome-view-site"></span></a>
								<?php endif; ?>

								<button class="show-details button">
									<p class="show-text"><?php echo esc_html__( 'Details', 'rt-easy-builder' ); ?></p>	
								...
								<?php
									if( rt_freemius()->is_not_paying() && $import_file[ 'pro' ] ){
										...
										<a href="//wpactivethemes.com/download/full-site-editing-themes/" target="_blank" class="ocdi__gl-item-button  button  button-primary buy-link">
											<?php esc_html_e( 'Buy', 'rt-easy-builder' ); ?><span class="dashicons dashicons-arrow-right-alt"></span>
										</a>
										<?php
									}
									...
									if( !rt_freemius()->is_not_paying() || !$import_file[ 'pro' ] ){
										...
										?>
										<a class="ocdi__gl-item-button  button  button-primary" href="<?php echo esc_url( $ocdi->get_plugin_settings_url( [ 'step' => 'import', 'import' => esc_attr( $index ) ] ) ); ?>">
											<?php esc_html_e( 'Import', 'rt-easy-builder' ); ?><span class="dashicons dashicons-download"></span>
										</a>
										<?php
									}
								?>
							...
							<div class="demo-requirement">
								...
								<div class="required-theme">
									<?php esc_html_e( 'Theme:', 'rt-easy-builder' ); ?> <a href="<?php echo esc_url( $import_file[ 'theme' ][ 'link' ] ); ?>" target="_blank"><?php echo esc_html( $import_file[ 'theme' ][ 'name' ] ); ?></a>
								</div>
								<?php if( $import_file['required_plugins'] ): ?>
									<div class="required-plugin">
										<?php esc_html_e( 'Required Plugins:', 'rt-easy-builder' ); ?>
										<ul>
