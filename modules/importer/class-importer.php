...
class Importer extends Builder{
	...
	public function __construct(){
		...
		$this->menu_title = esc_html__( 'Rise Demo Importer', 'rt-easy-builder' );
	...
	public function ajax_activate_plugins(){
		...
		if( ! current_user_can( 'manage_options' ) ){
			$this->ajax_response_error(
				__( 'You are not allowed perform this action.', 'rt-easy-builder' )
			);
			wp_die();
		}
	    ...
	    if( ! is_wp_error( $installed ) && $installed ){
			...
			$this->ajax_response_error(
				__( 'Something went wrong.', 'rt-easy-builder' )
			);
			wp_die();
		}
	}
	...
	public function create_plugin_page(){
		...
		$args = apply_filters( 'pt-ocdi/plugin_page_setup', array(
			...
			'page_title'  => esc_html__( 'Rise Demo Importer', 'rt-easy-builder' ),
	...
	public function get_import_files(){
		...
		$demos = wp_remote_get( $demo_json );
		$demos = wp_remote_retrieve_body( $demos );
				...
				<?php if ( $screenshot ) : ?>
					<div class="screenshot">
						<img src="<?php echo esc_url( $screenshot ); ?>" 
							alt="<?php esc_attr_e( 'Theme screenshot', 'rt-easy-builder' ); ?>" />
					</div>
				<?php else : ?>
	...
	public function after_import_setup() {
	    ...
	    $front_page_id = $this->get_page_by_title( 'Front Page' );
